...
void MakeTldY2(double* ptldY, double* pY, double* pA, double* pB, double* pnewT,
              int j, int S, int L, int J) {
  for (int s = 0; s < S; s++) {
    ptldY[s] = 0.0;
    for (int l = 0; l < L; l++) {
      double term = BTljsSum(pB,pnewT,s,l,j,S,L,J);
      ptldY[s] += pB[l+j*L]*(pY[l+s*L] - pA[l+s*L] - term);
    }
  }
}
/*************************************************************************/
...
void MakeGrvY2(double* pgrvY, double* pY, double* pA, double* pT, double* pnewB,
              double* pTSqRS, int j, int S, int L, int J) {
  for (int l = 0; l < L; l++) {
    double term1 = 0.0;
    for (int s = 0; s < S; s++) {
      double term2 = BTljsSum(pnewB,pT,s,l,j,S,L,J);
      term1 += pT[j+s*J]*(pY[l+s*L] - pA[l+s*L] - term2);
    }
    pgrvY[l] = term1/pTSqRS[j];
  }
}
...
void BC2(double* pnewB, double* pY, double* pA, double* pT, double rlam1, double rlam2,
  ...
  int niter = 0;
  while ((err>rthresh)&&(niter<imaxiter)&&(oldBSqS!=0.0)) {
    UpdateBC2(pnewB,pY,pA,pT,pTSqRS,rlam1,rlam2,S,L,J);
    err = MatErr(pnewB,poldB,L*J,rthresh);
    CopyAtoB(pnewB,poldB,L*J);
    oldBSqS = SqTotSum(poldB,L*J);
    niter++;
...
int TLatL2C2(double* pnewT, double* pY, double* pA, double* pB, double rthresh,
  ...
  int niter = 0;
  while ((err>rthresh)&&(niter<imaxiter)) {
    UpdateTLatL2C2(pnewT,pY,pA,pB,pBSqCS,rsT,S,L,J);
    err = MatErr(pnewT,poldT,J*S,rthresh);
    CopyAtoB(pnewT,poldT,J*S);
    niter++;
  }
...
void UpdateTLatL2C2(double* pnewT, double* pY, double* pA, double* pB,
  ...
  for (int j = 0; j < J; j++) {
    ...
    MakeTldY2(ptldY,pY,pA,pB,pnewT,j,S,L,J);
    double tldYSqS = 0.0, tldYnrm;
    for (int s = 0; s < S; s++) {
      tldYSqS += ptldY[s]*ptldY[s];
    }
...
/************************************************************************/
//Used for computing additive form of Welsch.
/************************************************************************/
void UpdateWelsch(double* pA, int S, int L, double sigma)
{
  for (int i = 0; i < L * S; ++i)
    pA[i] = pA[i] * ( 1 - std::exp( -(pA[i] * pA[i]) / (sigma * sigma) ) );
}

/************************************************************************/
//Used for computing additive form of Huber.
/************************************************************************/
void UpdateHuber(double* pA, int S, int L, double lambda)
{
  for (int i = 0; i < L * S; ++i)
    if (std::fabs(pA[i]) <= lambda)
      pA[i] = 0;
    else if (std::fabs(pA[i]) > lambda)
      pA[i] = pA[i] - lambda * ((pA[i] > 0)?1:-1);
}
                 ...
                 int L, int J, int S)
{
  ...
  for(int i = 0; i < L; ++i)
  {
    ...
    for(int s = 0; s < S; ++s)
    {
      for(int j = 0; j < J; ++j)
      {
        sum += pB[i * J + j] * pT[s + j * S];
      }
      pRes[i * S + s] =  pY[i * S + s] - sum;
    }
  }
}

/************************************************************************/
// Sigma is parameter of Welsch function
/************************************************************************/
double UpdateSigma(double *pQ, int S, int L, double dmul)
{
  double sum = 0, d = (S <= L) ? S:L;
  for(int i = 0; i < (S * L); ++i)
  {
    sum += (pQ[i] * pQ[i]);
  }
  return std::sqrt(sum / (dmul * d * d));
}

/************************************************************************/
// Lambda is parameter of Huber function
/************************************************************************/
double UpdateLambda(double *pQ, int S, int L, double alpha)
{
  std::vector<double> Q;
  for(int i = 0; i < (S * L); ++i)
  {
    Q.push_back( std::fabs(pQ[i]) );
  }
