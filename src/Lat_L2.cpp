...
/*
 * Our method with Welsch as a HQ function
 */
extern "C" SEXP LatL2C2(SEXP Y, SEXP nF, SEXP sigmul, SEXP inB, SEXP inT, SEXP lam1,
                      SEXP lam2, SEXP thresh, SEXP maxiter, SEXP maxiterB,
                      SEXP maxiterT,SEXP sT) {
  ...
  double *pY = REAL(Y), *pinB = REAL(inB), *pinT = REAL(inT),
    rlam1 = REAL(lam1)[0], rlam2 = REAL(lam2)[0],
    rthresh = REAL(thresh)[0], rsT = REAL(sT)[0], rsigmul = REAL(sigmul)[0], rsigma;
  int imaxiter = INTEGER(maxiter)[0], imaxiterB = INTEGER(maxiterB)[0],
                                            imaxiterT = INTEGER(maxiterT)[0];
  R_len_t S = ncols(Y), L = nrows(Y), J = INTEGER(nF)[0];

  //Initializing E.
  double *pE = new double[L*S];
  for (int i = 0; i < L * S; ++i)
    pE[i] = rand();
  //Initializing Beta.
  SEXP newB = R_NilValue;
  ...
  while (((perrBs[niter]>rthresh)||(perrTs[niter]>rthresh))&&
         (niter<imaxiter)&&(BSqS!=0.0)) {
    ComputeY_BT(pE,pY,pnewB,pnewT,L,J,S);
    rsigma =  UpdateSigma(pE,S,L,rsigmul);

    Rprintf("sigma = %lf\n", rsigma);
    UpdateWelsch(pE, S, L, rsigma);
    TLatL2C2(pnewT,pY,pE,pnewB,rthresh,imaxiterT,rsT,S,L,J);
    perrTs[niter+1] = MatErr(pnewT,poldT,J*S,rthresh);
    CopyAtoB(pnewT,poldT,J*S);
    BC2(pnewB,pY,pE,pnewT,rlam1,rlam2,rthresh,imaxiterB,S,L,J);
    perrBs[niter+1] = MatErr(pnewB,poldB,L*J,rthresh);
    CopyAtoB(pnewB,poldB,L*J);
    BSqS = SqTotSum(poldB,L*J);
    niter++;
...
/*
 * Our method with Huber as a HQ function
 */
extern "C" SEXP LatHuber(SEXP Y, SEXP nF, SEXP alphaHuber, SEXP inB, SEXP inT, SEXP lam1,
                       SEXP lam2, SEXP thresh, SEXP maxiter, SEXP maxiterB,
                       SEXP maxiterT,SEXP sT) {
  ...
  double *pY = REAL(Y), *pinB = REAL(inB), *pinT = REAL(inT),
    rlam1 = REAL(lam1)[0], rlam2 = REAL(lam2)[0],
                                             rthresh = REAL(thresh)[0], rsT = REAL(sT)[0], rAlphaHuber = REAL(alphaHuber)[0], rLambdaHuber;
  int imaxiter = INTEGER(maxiter)[0], imaxiterB = INTEGER(maxiterB)[0],
                                                                   imaxiterT = INTEGER(maxiterT)[0];
  R_len_t S = ncols(Y), L = nrows(Y), J = INTEGER(nF)[0];
  ...
  while (((perrBs[niter]>rthresh)||(perrTs[niter]>rthresh))&&
    ...
    // Huber parameter (lambda) is not adaptive yet
    rLambdaHuber = UpdateLambda(pE, S, L, rAlphaHuber);
    Rprintf("Lambda Huber = %lf\n", rLambdaHuber);
    /*
    Rprintf("\nBefore\n");
    for(int ii = 0; ii < L*J; ++ii)
      Rprintf("%lf, ", pE[ii]);
*/
    UpdateHuber(pE, S, L, rLambdaHuber);
    /*
    Rprintf("\nAfter\n");
    for(int ii = 0; ii < L*J; ++ii)
      Rprintf("%lf, ", pE[ii]);
...
static const R_CallMethodDef CallEntries[] = {
  ...
  {"LatL2C2", (DL_FUNC) &LatL2C2, 12},
  {"LatHuber", (DL_FUNC) &LatHuber, 12},
  {"TLatL2CR", (DL_FUNC) &TLatL2CR, 6},
  {NULL, NULL, 0}
};

extern "C" void R_init_RFLLat(DllInfo *dll) {
