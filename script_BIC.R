...
for (i in 1:num_files)
{
  ...
  setEPS()
  postscript(paste(paste(paste("./outputs/features_",fname), methods_name[k], sep="_"), "eps", sep = "."))
  plot(result.bic$opt.FLLat)

  ## Plot a heatmap of the weights for the optimal FLLat model.
  postscript(paste(paste(paste("./outputs/features_heatmap_",fname), methods_name[k], sep="_"), "eps", sep = "."))
